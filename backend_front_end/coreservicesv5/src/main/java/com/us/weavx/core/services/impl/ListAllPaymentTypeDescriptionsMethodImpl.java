...
@Component
public class ListAllPaymentTypeDescriptionsMethodImpl implements ServiceMethod {
	@Autowired
	private ConfigurationTxServices services;

	public Response executeMethod(Request request) {
		try {
			List<PaymentTypeDescription> paymentTypes = services.listAllPaymentTypeDescriptions();
			Response res = new Response();
			res.setReturnCode(ServiceReturnMessages.SUCCESS_CODE);
			res.setReturnMessage(ServiceReturnMessages.SUCCESS);
			HashMap<String, Object> result = new HashMap<>();
			result.put("paymentTypeDescriptions", paymentTypes);

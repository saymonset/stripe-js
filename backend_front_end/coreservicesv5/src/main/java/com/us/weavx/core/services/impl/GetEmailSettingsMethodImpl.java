...
@Component
public class GetEmailSettingsMethodImpl implements ServiceMethod {
	@Autowired
	private ConfigurationTxServices configurationServices;

	public Response executeMethod(Request request) {
		try {
			List<SystemProperty> systemProperties = configurationServices.getSystemProperties();
			List<SystemProperty> emailSettings = new ArrayList<>();
			for (SystemProperty prop : systemProperties) {
				if ((prop.getName().indexOf("SES_") != -1) || (prop.getName().indexOf("SMTP") != -1)) {
					emailSettings.add(prop);
				}
			}

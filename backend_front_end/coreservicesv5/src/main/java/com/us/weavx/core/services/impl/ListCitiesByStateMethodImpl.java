...
@Component
public class ListCitiesByStateMethodImpl implements ServiceMethod {
	@Autowired
	private ConfigurationTxServices configurationServices;

	public Response executeMethod(Request request) {
		try {
			HashMap<String, Object> parameters = request.getParameters();
			Integer stateId = (Integer) parameters.get("stateId");
			List<City> resultTmp = configurationServices.listCitiesByState(stateId);			
			Response res = new Response();
			res.setReturnCode(ServiceReturnMessages.SUCCESS_CODE);
			res.setReturnMessage(ServiceReturnMessages.SUCCESS);
			HashMap<String, Object> result = new HashMap<>();
			result.put("cities", resultTmp);

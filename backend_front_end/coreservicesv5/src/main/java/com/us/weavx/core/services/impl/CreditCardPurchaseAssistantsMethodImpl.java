...
@Component
public class CreditCardPurchaseAssistantsMethodImpl implements ServiceMethod {
		...
		try {
			...
			while (iter.hasNext()) {
				...
				if (f.getCustomerId() != customerId) {
					...
					String fundAmountStr = parameterManager.getApplicationParameter(applicationId, Integer.toString(f.getId())); 
			...
			int paymentGatewayId = (Integer) paymentGwInfoParam.get("paymentGatewayId");
			HashMap<String, Object> gwParameters = (HashMap<String, Object>) paymentGwInfoParam.get("paymentGwParameters");
			PaymentGatewayInfo pGwInfo = new PaymentGatewayInfo();
			pGwInfo.setPaymentGatewayId(paymentGatewayId);
			pGwInfo.setPaymentGwParameters(gwParameters);
			...
			//Se obtienen los asistentes enviados los asistentes
			List<HashMap<String, Object>> assistantsListMap = params.get("assistants") != null ? (List<HashMap<String, Object>>) params.get("assistants") : new ArrayList<HashMap<String,Object>>();
			List<Assistant> assistants = new ArrayList<>();
			for (HashMap<String, Object> assistantMap : assistantsListMap) {
				Assistant assistant = new Assistant(assistantMap.get("firstName").toString(), 
						assistantMap.get("lastName").toString(),  
						assistantMap.get("email").toString(), 
						assistantMap.get("phoneNumber").toString(), 
						assistantMap.get("address").toString(), 
						Integer.parseInt(assistantMap.get("countryId").toString()), 
						transaction.getId());
				assistants.add(assistant);
			}
			
			PaymentResult paymentResult = transactionServices.proccessCreditCardTransaction(transaction, transactionDetails, pGwInfo, txUserData, userToken, langId, couponSerial, userAgent, ipAddress, assistants);
			Response res = new Response();
			...
			case PaymentResult.ABORTED_TRANSACTION:
				res = new Response();
				if (paymentResult.getResultMessage().indexOf("blacklist")!= -1) {
					res.setReturnCode(ServiceReturnMessages.FRAUD_TRANSACTION_CODE);
					res.setReturnMessage(ServiceReturnMessages.FRAUD_TRANSACTION+paymentResult.getResultMessage());
				} else {
				...
				res.setResult(result);
				return res;	
			case PaymentResult.FRAUD_TRANSACTION:
				res = new Response();
				res.setReturnCode(ServiceReturnMessages.FRAUD_TRANSACTION_CODE);
				res.setReturnMessage(ServiceReturnMessages.FRAUD_TRANSACTION+paymentResult.getResultMessage());
				result.put("paymentResult", paymentResult);

...
@Component
public class ListAllExternalPaymentTypesMethodImpl implements ServiceMethod {
	@Autowired
	private ConfigurationTxServices configurationServices;

	public Response executeMethod(Request request) {
		try {
			List<ExternalPaymentType> resultTmp = configurationServices.listAllExternalPaymentTypes();
			Response res = new Response();
			res.setReturnCode(ServiceReturnMessages.SUCCESS_CODE);
			res.setReturnMessage(ServiceReturnMessages.SUCCESS);
			HashMap<String, Object> result = new HashMap<>();
			result.put("externalPaymentTypes", resultTmp);

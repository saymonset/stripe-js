CREATE OR REPLACE FUNCTION UPDATE_REPORTS() RETURNS void AS $$
	...
	insert into transaction_report(customer_id, application_id, application_name, person_firstname, 
	       ...
	       donation_campaing, donation_Campaing_id, transaction_medium, transaction_medium_id, comments, operator_id, operator_email, person_phone, ip_address, user_agent)
	...
	tmed.name, tmed.id, t.comments, t.operator_id, t.operator_email, coalesce(cu.phone_number,''), t.ip_address, t.user_agent from transaction t inner join transaction_status ts on ts.id = t.tx_status_id 
	...
	inner join application app on t.application_id = app.id inner join payment_data pd on t.tx_payment_data_id = pd.id 
	left join payment_gateway pgw on t.tx_payment_gw_id = pgw.id inner join transaction_source tsour on t.tx_transaction_source_id = tsour.id 
	inner join transaction_campaing tcamp on t.tx_transaction_campaing_id = tcamp.id inner join transaction_medium tmed on t.tx_transaction_medium_id = tmed.id, fund f, payment_type pt
	where td.fund_id = f.id and pd.payment_type_id = pt.id and t.tx_date between last_run_date and current_date_var
	order by t.tx_id;
